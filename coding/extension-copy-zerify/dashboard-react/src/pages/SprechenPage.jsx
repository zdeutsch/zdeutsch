...
function DiscussionTopics({ topics = [], onChange }) {
  ...
  return (
    <Section title="Diskussionsthemen" description={`${topics.length} Themen mit Titel und Text`} action={<AddButton onClick={() => onChange([...topics, { title: "", text: "" }])}>Thema hinzufügen</AddButton>}>
      <div className="item-list">{topics.map((topic, index) => <details className="topic-editor" key={index}><summary><span><MessagesSquare size={16} /><strong>{topic.title || `Thema ${index + 1}`}</strong></span></summary><div className="topic-editor__body"><div className="form-grid"><Field label="Titel" className="field--wide"><input value={topic.title || ""} onChange={(event) => update(index, { title: event.target.value })} /></Field><Field label="Text" className="field--wide"><textarea rows="12" value={topic.text || ""} onChange={(event) => update(index, { text: event.target.value })} /></Field></div><div className="item-actions"><button className="button button--danger-ghost button--small" type="button" onClick={() => onChange(topics.filter((_, itemIndex) => itemIndex !== index))}>Thema entfernen</button></div></div></details>)}</div>
    </Section>
  );
}
